...
export const loadAppDetails = createAsyncThunk(
  ...
  async ({ networkID, provider }: IBaseAsyncThunk, { dispatch }) => {
    ...
    const sHecMainContract = new ethers.Contract(addresses[networkID].ECOIN_ADDRESS as string, sOHMv2, provider);
    const hecContract = new ethers.Contract(addresses[networkID].ROBOT_ADDRESS as string, ierc20Abi, provider);
    // const oldsHecContract = new ethers.Contract(
    //   addresses[networkID].OLD_ECOIN_ADDRESS as string,
    //   [circulatingSupply],
    //   provider,
    // );
...
const loadMarketPrice = createAsyncThunk("app/loadMarketPrice", async ({ networkID, provider }: IBaseAsyncThunk) => {
  ...
  try {
    ...
    marketPrice = await getTokenPrice("robot");
  }
  return { marketPrice };
});
