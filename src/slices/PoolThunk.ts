...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const ecoinContract = new ethers.Contract(addresses[networkID].ECOIN_ADDRESS, ierc20Abi, signer);

    let approveTx;
    let depositAllowance = await ecoinContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
    ...
    depositAllowance = await ecoinContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
