...
contract ROBOTTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, ECOIN }
    ...
    address public ECOIN;
    uint public ECOINQueue; // Delays change to ECOIN address
    ...
    /**
        @notice allow approved address to deposit an asset for ROBOT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ROBOT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ROBOT ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ROBOT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IROBOTERC20( ROBOT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( ECOIN ).balanceOf( msg.sender ); // Can only borrow against ECOIN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ROBOT
        @param _amount uint
     */
    function repayDebtWithROBOT( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IROBOTERC20( ROBOT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ROBOT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ROBOT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ROBOT ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ROBOT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ROBOT decimals
            value_ = _amount.mul( 10 ** IERC20( ROBOT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.ECOIN ) { // 9
            ECOINQueue = block.number.add( blocksNeededForQueue );
        } else return false;
